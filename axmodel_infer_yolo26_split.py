...
class YOLO26Detector:
    ...
    def detect_objects(self, image, save_path):
        ...
        outputs = self.session.run(self.output_names, {self.session.get_inputs()[0].name: im})
        outputs = [np.transpose(outputs[i],(0,3,1,2)) for i in range(2*self.nl)]    #to nchw
        # outputs 为交替的 [box0, cls0, box1, cls1, box2, cls2]
        box_outputs = [outputs[i] for i in range(0, len(outputs), 2)]  # box分支
        cls_outputs = [outputs[i] for i in range(1, len(outputs), 2)]  # cls分支
        
        # 为make_anchors构造特征列表（使用box的shape信息）
        feats_for_anchors = box_outputs
        anchors, strides = (np.transpose(x_arr, (1, 0)) for x_arr in make_anchors(feats_for_anchors, self.stride, 0.5))
        
        # 提取box和cls
        # box_outputs: [1, 64, h, w] → reshape为 [1, 4*reg_max, -1]
        box = []
        for i, b_out in enumerate(box_outputs):
            # b_out shape: [1, 64, h, w]
            # 需要转换为 [1, 4*reg_max, -1] 格式
            b_out_reshaped = b_out.reshape(1, 4 * self.reg_max, -1)  # [1, 64, h*w]
            box.append(b_out_reshaped)
        
        # cls_outputs: [1, 1, h, w] → reshape为 [1, nc, -1]
        cls = []
        for i, c_out in enumerate(cls_outputs):
            # c_out shape: [1, 1, h, w] 或 [1, num_cls, h, w]
            c_out_reshaped = c_out.reshape(1, self.nc, -1)  # [1, nc, h*w]
            cls.append(c_out_reshaped)
        
        # 合并所有分支
        boxes = np.concatenate([box[i] for i in range(self.nl)], axis=-1)  # [1, 64, total_anchors]
        scores = np.concatenate([cls[i] for i in range(self.nl)], axis=-1)  # [1, nc, total_anchors]
        ...
        # 数值稳定的sigmoid，避免溢出
        scores_sigmoid = 1 / (1 + np.exp(-np.clip(scores, -500, 500)))
        
        # 合并框和分数
        y = np.concatenate((dbox, scores_sigmoid), axis=1)  # [1, 5, total_anchors]
        y = y.transpose([0, 2, 1])  # [1, total_anchors, 5]
        
        # 后处理
        pred = self.postprocess(y)
...
if __name__ == '__main__':
    ...
    detector = YOLO26Detector(model_path='./yolo26s_drone_650_s.axmodel', imgsz=[640,640])
    img_path = './drone_pic'
    det_path='./drone_yolo26_res'
